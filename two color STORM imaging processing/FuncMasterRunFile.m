function FuncMasterRunFile(mainDirFolder, MolFolder,FileName, BeadMolFolder)
%%Cat 2 (680), Cat 1 (647) 
...
alist_r = [mainDirPath FileName '-R_alist.bin'];
alist_l = [mainDirPath FileName '-L_alist.bin'];
mlist_r = [mainDirPath FileName '-R_mlist.bin'];
mlist_l = [mainDirPath FileName '-L_mlist.bin'];
...
% 
for i = 5000:5500 % For total of i frames. Build distance matrix for all molecules start in frame i in lmol_alist with all molecules in frame i in rmol (mlist).
...
outfile = sprintf('%s_Split-L_H%g.bin',filehead,XTol);
outfileLR = sprintf('%s_Split-LR_H%g.bin',filehead,XTol);
...
%             [txoR,tyoR] = tforminv(tformrev,SplitData.xo(i),SplitData.yo(i));
        	...
        	backgroundL = sum(sum(BgKernel.*double(movie((Currentyl-5):(Currentyl+5),(Currentxl-5):(Currentxl+5),CurrentFrame))));
            backgroundR = sum(sum(BgKernel.*double(movie((Currentyr-5):(Currentyr+5),(Currentxr-5):(Currentxr+5),CurrentFrame))));
            IntensityL = sum(sum(SignalKernel.*double(movie((Currentyr-1):(Currentyr+1),(Currentxr-1):(Currentxr+1),CurrentFrame))))-backgroundL;
            IntensityR = sum(sum(SignalKernel.*double(movie((Currentyr-1):(Currentyr+1),(Currentxr-1):(Currentxr+1),CurrentFrame))))-backgroundR;
